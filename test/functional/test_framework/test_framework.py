...
class BitcoinTestFramework():
    ...
    def main(self):
        ...
        parser.add_option("--nocleanup", dest="nocleanup", default=False, action="store_true",
                          help="Leave lksds and test.* datadir on exit or error")
        parser.add_option("--noshutdown", dest="noshutdown", default=False, action="store_true",
                          help="Don't stop lksds after the test execution")
        parser.add_option("--srcdir", dest="srcdir", default=os.path.normpath(os.path.dirname(os.path.realpath(__file__)) + "/../../../src"),
                          help="Source directory containing lksd/lks-cli (default: %default)")
        ...
        parser.add_option("--usecli", dest="usecli", default=False, action="store_true",
                          help="use lks-cli instead of RPC for all commands")
        parser.add_option("--lksd-arg", dest="lksd_extra_args", default=[], type='string', action='append',
                          help="Pass extra args to all lksd instances")
        ...
        self.extra_args_from_options = self.options.lksd_extra_args
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: lksds were not stopped and may still be running")
    ...
    def stop_node(self, i, wait=0):
        """Stop a lksd test node"""
        self.nodes[i].stop_node(wait=wait)
        self.nodes[i].wait_until_stopped()

    def stop_nodes(self, wait=0):
        """Stop multiple lksd test nodes"""
        for node in self.nodes:
    ...
    def assert_start_raises_init_error(self, i, extra_args=None, expected_msg=None, *args, **kwargs):
        with tempfile.SpooledTemporaryFile(max_size=2**16) as log_stderr:
            try:
                ...
                assert 'lksd exited' in str(e)  # node must have shutdown
                ...
                if expected_msg is None:
                    assert_msg = "lksd should have exited with an error"
                else:
                    assert_msg = "lksd should have exited with expected error " + expected_msg
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as lksd's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self, extra_args=None, stderr=None):
        ...
        if create_cache:
            ...
            # Create cache directories, run lksds:
            self.set_genesis_mocktime()
            for i in range(MAX_NODES):
                datadir = initialize_datadir(self.options.cachedir, i)
                args = [os.getenv("LKSD", "lksd"), "-server", "-keypool=1", "-datadir=" + datadir, "-discover=0", "-mocktime="+str(GENESISTIME)]
                if i > 0:
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i)  # Overwrite port/rpcport in lks.conf
...
class LksTestFramework(BitcoinTestFramework):
    def set_lks_test_params(self, num_nodes, masterodes_count, extra_args=None, fast_dip3_enforcement=False):
    ...
    def set_lks_dip8_activation(self, activate_after_block):
    ...
    def set_lks_llmq_test_params(self, llmq_size, llmq_threshold):
