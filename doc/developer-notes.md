...
Lksc Core is a multithreaded application, and deadlocks or other multithreading bugs
...
([`valgrind.supp`](https://github.com/LKSCOIN/LKSCOIN/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_lks
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_lks --log_level=test_suite
$ valgrind -v --leak-check=full src/lksd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_lks.coverage/index.html`.
```
...
- ThreadSocketHandler : Sends/Receives data from peers on port 9400.
...
However, in open source software such as Lksc Core, where everyone uses
...
pay attention to for reviewers of Lksc Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Lksc Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Lksc Core so that they can be integrated
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `lks-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `lks-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Lksc Core, the wallet was always
