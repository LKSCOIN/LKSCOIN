...
------------------------------
In the past, the build documentation contained instructions on how to build Lks with system-wide installed dependencies
...
---------------------
Lks inherited the `depends` folder from Bitcoin, which contains all dependencies required to build Lks. These
dependencies must be built before Lks can actually be built. To do so, perform the following:
...
not done, build failures might occur when building Lks.
