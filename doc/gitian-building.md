...
*Setup instructions for a Gitian build of Lksc Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Lksc
...
- [Building Lksc Core](#building-lks-core)
...
Clone the git repositories for Lksc Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/LKSCOIN/LKSCOIN
...
At this point you have two options, you can either use the automated script (found in [contrib/gitian-build.py](/contrib/gitian-build.py)) or you could manually do everything by following this guide. If you're using the automated script, then run it with the "--setup" command. Afterwards, run it with the "--build" command (example: "contrib/gitian-building.sh -b signer 3.1.0"). Otherwise ignore this.

Follow the instructions in [doc/release-process.md](release-process.md#fetch-and-create-inputs-first-time-or-when-dependency-versions-change)
in the Lksc Core repository under 'Fetch and create inputs' to install sources which require
...
To build Lksc Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Lksc Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/lks/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/LKSCOIN/LKSCOIN
...
For example:
```bash
URL=https://github.com/crowning-/lks.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit lks=${COMMIT} --url lks=${URL} ../lks/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit lks=${COMMIT} --url lks=${URL} ../lks/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit lks=${COMMIT} --url lks=${URL} ../lks/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the lks git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../lks/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/lks
SIGPATH=/some/root/path/lks-detached-sigs

./bin/gbuild --url lks=${BTCPATH},signature=${SIGPATH} ../lks/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/lks-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/lks-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/lks-osx-build.assert
```
