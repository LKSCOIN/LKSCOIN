Bitcoin Core version 0.2.0 is now available from:

  <https://bitcoin.org/bin/bitcoin-core-0.2.0/>
...
### Downgrade to a version < 0.2.0

Because release 0.2.0 and later will obfuscate the chainstate on every
fresh sync or reindex, the chainstate is not backwards-compatible with
pre-0.2 versions of Bitcoin Core or other software.

If you want to downgrade after you have done a reindex with 0.2.0 or later,
...
Bitcoin Core 0.2 will have a strict maximum size on the mempool. The
...
Bitcoin Core 0.2 also introduces new default policy limits on the length and
...
Bitcoin Core 0.2 nodes. Bitcoin Core will only allow replacement of
...
Note that the wallet in Bitcoin Core 0.2 does not yet have support for
...
In Bitcoin Core 0.2, when mempool limit has been reached a higher minimum
...
sanity check. Since 0.2, these are no longer stored. When loading a
0.2 wallet into an older version, it will automatically rescan to avoid
failed checks.

Wallet: Pruning
---------------

With 0.2 it is possible to use wallet functionality in pruned mode.
...
The mining code in 0.2 has been optimized to be significantly faster and use less
...
- #7323 `a344880` 0.2: Backport -bytespersigop option (Luke-Jr)
...
- #6373 `1ae3196` depends: non-qt bumps for 0.2 (Cory Fields)
