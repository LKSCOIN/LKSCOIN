...
// which are part of the Lksc repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="lks-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /lks-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "lkspay-lks/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from lkspay-lks/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'lkspay-lks/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from lkspay-lks/develop"
            }
          }

          if (hasCache) {
            sh "cd /lks-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /lks-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /lks-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /lks-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /lks-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /lks-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /lks-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
