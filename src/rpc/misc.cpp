...
UniValue getinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total lks balance of the wallet\n"
...
UniValue debug(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            " - \"lks\" activates all Lksc-specific categories at once;\n"
...
#ifdef ENABLE_WALLET
        // Case 1: Lksc address and we have full public key:
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
    {
        std::string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are lks addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) lks address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
{ //  category              name                      actor (function)         okSafeMode
  //  --------------------- ------------------------  -----------------------  ----------
    ...
    /* Lksc features */
    { "lks",               "mnsync",                 &mnsync,                 true,  {} },
    { "lks",               "spork",                  &spork,                  true,  {"value"} },
