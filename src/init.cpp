...
public:
    ...
    // Lks Specific WalletInitInterface InitPrivateSendSettings
 ...
 * between lksd, and lks-qt and non-server tools.
*/
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    if (mode == HMM_BITCOIN_QT)
        strUsage += HelpMessageOpt("-windowtitle=<name>", _("Sets a window title which is appended to \"Lksc Core - \""));
    ...
    strUsage += HelpMessageOpt("-sporkaddr=<lksaddress>", strprintf(_("Override spork address. Only useful for regtest and devnet. Using this on mainnet or testnet will ban you.")));
...
std::string LicenseInfo()
{
    ...
    const std::string URL_WEBSITE = "<https://www.lksfoundation.org>";
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile, because if Lksc Core is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if Lksc Core is started while in a temporary directory.\n",
    ...
    // ********************************************************* Step 10c: schedule Lks-specific tasks
