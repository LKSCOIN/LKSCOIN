...
const char * const BITCOIN_CONF_FILENAME = "lks.conf";
const char * const BITCOIN_PID_FILENAME = "lksd.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "lksd -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\LksCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\LksCore
    // Mac: ~/Library/Application Support/LksCore
    // Unix: ~/.lkscore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "LksCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/LksCore";
#else
    // Unix
    return pathRet / ".lkscore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty lks.conf if it does not excist
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    // Check for untranslated substitution to make sure Lksc Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Lksc Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2014, nEndYear) + "The Lksc Core developers";
    }
