    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "lks" is a composite category enabling all Lksc-related debug output
                if(ptrCategory->count(std::string("lks"))) {
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty lks.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override lks.conf
