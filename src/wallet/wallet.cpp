...
#include "txmempool.h"
#include "util.h"
#include "init.h" //aggiunto Omar Mod.
...
CAmount CWalletTx::GetAnonymizedCredit(bool fUseCache) const
{
    ...
    fAnonymizedCreditCached = false;//true orig
    return nCredit;
}
...
bool CWallet::SelectCoinsGrouppedByAddresses(std::vector<CompactTallyItem>& vecTallyRet, bool fSkipDenominated, bool fAnonymizable, bool fSkipUnconfirmed) const
{
    ...
    fAnonymizable=false; //or. non c'era intera stringa, aggiunta
    ...
    if(fAnonymizable && fSkipUnconfirmed) {
        if(fSkipDenominated) {
            ...
            fAnonymizableTallyCachedNonDenom = false;//or true
        } else {
            vecAnonymizableTallyCached = vecTallyRet;
            fAnonymizableTallyCached = false; // or true
        }
    }
...
bool CWallet::CreateTransaction(const vector<CRecipient>& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet,
	int& nChangePosRet, std::string& strFailReason, const CCoinControl* coinControl, bool sign, AvailableCoinsType nCoinType, bool fUseInstantSend, string addressrepay)
{
...

    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                double dPriority = 0;
                // vouts to the payees 
	            CScript pubkeytosend;
                BOOST_FOREACH (const CRecipient& recipient, vecSend)
                {
	                pubkeytosend = recipient.scriptPubKey; // Mod. Aggiunta Omar
	                CTxOut txout(recipient.nAmount, recipient.scriptPubKey); //Tx out verso indirizzo di invio richiesto
                ...
                if (fUseInstantSend && nValueIn > sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE)*COIN) {
                    strFailReason += " " + strprintf(_("InstantSend doesn't support sending values that high yet. Transactions are currently limited to %1 LKS."), sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE));
                    return false;
                }
	            
	            //nValueIn = nValueToSelect;//Aggiunto Omar Mod.
                ...
                if (nChange > 0)
                {
                    ...
                    if (nCoinType == ONLY_DENOMINATED) { 
	                    ...
	                    if (addressrepay != "")
	                    {
		                    CBitcoinAddress address(addressrepay);
		                    CScript pubkeytoreceveeccedence = GetScriptForDestination(address.Get());
		                    newTxOut = CTxOut(nChange, pubkeytoreceveeccedence);
	                    }
	                    else
	                    {
                        ...
                        else
                        {
                            ...
                            vector<CTxOut>::iterator position = txNew.vout.begin()+nChangePosRet;//{118 'v', -87 '�', 20 '\024', 80 'P', -94 '�', ...}
