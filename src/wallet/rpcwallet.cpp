...
// Copyright (c) 2017-2020 The Lksc Core developers
...
static void SendMoney(CWallet * const pwallet, const CTxDestination &address, CAmount nValue, bool fSubtractFeeFromAmount, CWalletTx& wtxNew, const CCoinControl& coin_control, std::string addressrefound="") 
{
    ...
    if (!pwallet->CreateTransaction(vecSend, wtxNew, reservekey, nFeeRequired, nChangePosRet,
                                         strError, coin_control, addressrefound)) {
...
UniValue sendtoaddress(const JSONRPCRequest& request)
{
    ...
    SendMoney(pwallet, dest, nAmount, fSubtractFeeFromAmount, wtx, coin_control, returnfounds); 
...
UniValue sendfrom(const JSONRPCRequest& request)
{
    ...
    SendMoney(pwallet, dest, nAmount, false, wtx, no_coin_control, returnfounds); 
...
UniValue sendmany(const JSONRPCRequest& request)
{
    ...
    for (const std::string& name_ : keys) {
        ...
        if (!IsValidDestination(dest)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Lks address: ") + name_);
        }
...
UniValue addmultisigaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
    {
        std::string msg = "addmultisigaddress nrequired [\"key\",...] ( \"account\" )\n"
            ...
            "}\n"
            "\nResult (DEPRECATED. To see this result in v3.3 instead, please start lksd with -deprecatedrpc=addmultisigaddress).\n"
...
UniValue loadwallet(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nNote that all wallet command-line options used when starting lksd will be"
...
UniValue listunspent(const JSONRPCRequest& request)
{
    ...
    if (!request.params[2].isNull()) {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++) {
            ...
            if (!IsValidDestination(dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Lks address: ") + input.get_str());
            }
    ...
    if (!request.params[1].isNull()) {
      if (request.params[1].type() == UniValue::VBOOL) {
        ...
        RPCTypeCheckObj(options,
            {
                ...
                {"reserveChangeKey", UniValueType(UniValue::VBOOL)}, // DEPRECATED (and ignored), should be removed in 3.3 or so.
        ...
        if (options.exists("changeAddress")) {
            ...
            if (!IsValidDestination(dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "changeAddress must be a valid lks address");
            }
