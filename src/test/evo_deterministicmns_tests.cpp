...
BOOST_FIXTURE_TEST_CASE(dip3_protx, TestChainDIP3Setup)
{
    ...
    // test ProUpServTx
    auto tx = CreateProUpServTx(utxos, dmnHashes[0], operatorKeys[dmnHashes[0]], 100000, CScript(), coinbaseKey); 
    CreateAndProcessBlock({tx}, coinbaseKey);
    deterministicMNManager->UpdatedBlockTip(chainActive.Tip());
    BOOST_ASSERT(chainActive.Height() == nHeight + 1);
    nHeight++;
    ...
    BOOST_ASSERT(dmn != nullptr && dmn->pdmnState->addr.GetPort() == 100000); 
