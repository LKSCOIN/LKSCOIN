...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "LksNetwork") type = LksNetwork;
    if(strType == "LksNetworkVariable") type = LksNetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == LksNetwork) s = "LksNetwork";
    if(type == LksNetworkVariable) s = "LksNetworkVariable";
