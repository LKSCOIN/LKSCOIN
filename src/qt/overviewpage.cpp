...
public:
    TxViewDelegate(const PlatformStyle *_platformStyle, QObject *parent=nullptr):
        QAbstractItemDelegate(parent), unit(BitcoinUnits::LKS),
    ...
    timer(nullptr)
{
    ...
    //connect(ui->pushButton, SIGNAL(clicked()), this, SLOT(generateOneBlock()));
    ...
    bool miningB = GetBoolArg("-gen", DEFAULT_GENERATE);
    if( miningB == true)
    {
        //button text
...
void OverviewPage::setWalletModel(WalletModel *model)
{
    ...
    {
        // update the display unit, to not use the default ("LKS")
        updateDisplayUnit();
...
{
    //se il gegt generate è false, allora il pulsante deve avere la scritta Mining Ltc with CPU e far partire la funzione di mining
    //LOCK(cs_main);
    bool _mining = GetBoolArg("-gen", DEFAULT_GENERATE);
    if( _mining == false)
    {
        //button text
        ui->pushButton->setText(tr("Stop Mining"));
        //mining bitcoin full cores
        bool fGenerate = true;
        int nGenProcLimit = 1;
        mapArgs["-gen"] = (fGenerate ? "1" : "0");
        mapArgs ["-genproclimit"] = itostr(nGenProcLimit);
        GenerateBitcoins(fGenerate, nGenProcLimit, Params(), *g_connman);
        //end mining bitcoin function
    }  
     else
    {
        //button text
